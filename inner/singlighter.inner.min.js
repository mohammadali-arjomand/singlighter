...
export default class {{PAGENAME}} extends Page {
	...
	'${command}' is not a command`)}function errorHelper(e){console.error("\x1b[31m%s\x1b[0m",e)}function successHelper(e){console.log("\x1b[32m%s\x1b[0m",e)}function makePageCmd(e){fs.writeFileSync(`./Scripts/Pages/${e}.js`,archive.page.replace("{{PAGENAME}}",e)),successHelper("Page was created successfully")}function serveCmd(){successHelper("Server was started on [http://localhost:8000] - Press Ctrl+C for stop server");let e=(e,r)=>{e.url="."+e.url,fs.existsSync(e.url)||(e.url="/"),url=fs.lstatSync(e.url).isFile()?"./"+e.url:`./${e.url}/index.html`;let s=fs.readFileSync(url);r.write(s),url.substring(url.length-2,url.length)==js&&r.writeHead(200,{"Content-type":"application/javascript"}),r.end()},r=http.createServer(e).listen(8e3);fs.watch(".",()=>{successHelper("Files was modified, Server was restarted"),r.close(),r=http.createServer(e).listen(8e3)})}